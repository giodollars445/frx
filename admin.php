...
// Get unique dates for prenotazioni filter (only dates with bookings)
    ...
    fetch('get_booking_dates.php')
        ...
        .then(data => {
            if (data.success && data.dates) {
                ...
                // Clear dropdown keeping only "All dates" option
                ...
                if (currentValue) {
                    ...
                    if (optionToSelect) {
                        ...
                        // If the previously selected date no longer exists, reset filter
                        dateSelect.value = '';
                        currentDateFilter = '';
                    }
                }
                ...
                if (currentDateFilter) {
                    const optionExists = Array.from(dateSelect.options).some(
                        opt => opt.value === currentDateFilter
                    );
    ...
    fetch('get_bookings_table.php?exclude_cancelled=1')
        ...
        .then(html => {
            if (html !== lastRenderedTableHTML) {
                ...
                updateStats();
                updateDateFilterDropdown(); // Update the dropdown dates if needed
                
                // If we have an active filter, check if we need to reset it
                if (currentDateFilter) {
                    const rowsWithData = document.querySelectorAll('.table-container table tbody tr[style=""]');
                    if (rowsWithData.length === 0) {
                        document.getElementById('data_prenotazioni').value = '';
                        currentDateFilter = '';
                    }
                }
...
function filtraPrenotazioni(data) {
    ...
    // If no bookings found for selected date, reset the filter
    if (data && !hasVisibleRows) {
        document.getElementById('data_prenotazioni').value = '';
        currentDateFilter = '';
        // Show all rows again
